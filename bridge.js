...
const DEFAULTS = {
  ...
  hoodmaps: {
    ...
    categories: {
      hipsters: true, uni: true, rich: true, suits: true,
      normies: true, tourists: true, nightlife: true, crime: true,
    },
  },
};
...
function merge(saved) {
  ...
  return {
    ...
    transit: {
      ...DEFAULTS.transit,
      ...(saved.transit || {}),
      modes: { ...DEFAULTS.transit.modes, ...((saved.transit && saved.transit.modes) || {}) },
    },
    hoodmaps: {
      ...DEFAULTS.hoodmaps,
      ...(saved.hoodmaps || {}),
      categories: { ...DEFAULTS.hoodmaps.categories, ...((saved.hoodmaps && saved.hoodmaps.categories) || {}) },
    },
  };
}

chrome.storage.local.get(SETTINGS_KEY).then(({ [SETTINGS_KEY]: saved }) => sendSettings(merge(saved)));
...
window.addEventListener("message", async (e) => {
  if (e.source !== window || !e.data || e.data.source !== "abnb-transit-overlay-page") return;
  const slug = e.data.slug;
  if (!slug) return;

  if (e.data.type === "tagsRequest") {
    try {
      const j = await cachedFetchJSON("hoodmaps:" + slug,
        "https://hoodmaps.com/?action=get_data&slug=" + encodeURIComponent(slug));
      if (!j || !Array.isArray(j.tags)) throw new Error("malformed");
      const tags = j.tags
        .filter((t) => t && t.tag && Number.isFinite(t.latitude) && Number.isFinite(t.longitude))
        .map((t) => ({
          lat: t.latitude, lng: t.longitude, text: t.tag,
          votes: t.votes | 0, sentiment: t.sentiment_score | 0,
        }));
      window.postMessage({ source: "abnb-transit-overlay", type: "tagsResponse", slug, tags }, "*");
    } catch (err) {
      window.postMessage({ source: "abnb-transit-overlay", type: "tagsResponse", slug, error: String(err) }, "*");
    }
  } else if (e.data.type === "districtsRequest") {
    try {
      const geojson = await cachedFetchJSON("hoodmaps-districts:" + slug,
        "https://hoodmaps.com/assets/districts_categorized/" + encodeURIComponent(slug) + ".geojson");
      window.postMessage({ source: "abnb-transit-overlay", type: "districtsResponse", slug, geojson }, "*");
    } catch (err) {
      window.postMessage({ source: "abnb-transit-overlay", type: "districtsResponse", slug, error: String(err) }, "*");
    }
  }
});
